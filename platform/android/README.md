...
| --- | --- |
| `ble` | `AndroidBeaconRadio` (core `BeaconRadio`: advertising sets, shared filtered scan), `AdvertisingPlan`, `ScanPlan`, `ScanScheduler` (5 starts per 30 s), `ScanRecordAdapter`, `BluetoothPowerMonitor`, `BeaconRadioState` |
...
| `bluetooth.gatt` | `GattStreamChannel` and its segment format (`GattSegments`: credits, sequence numbers), `GattClientLink`, `GattServerHost` |
