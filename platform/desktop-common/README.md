...
| `data.DesktopDatabase` | `DropData` over the SQLite JDBC driver, secrets field-encrypted under `DatabaseKeys`. The 24 h clean-up (`PartialsSweeper`, §7.7) and the resume adapter live in `platform/common` (shared with Android). |
...
not loop back), LAN links and the addresses they may join, folder expansion (limit, long names,
