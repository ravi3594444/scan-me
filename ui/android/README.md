...
| File | Role |
| --- | --- |
| `MainActivity` | Hosts `DropApp`; `ACTION_SEND` / `ACTION_SEND_MULTIPLE` for any type; activity-result launchers; Back through the controller; reduced motion from "Remove animations" |
| `DropApplication`, `AppGraph` | One `DropAppController` per process and the ports behind it |
...
| `MediaStoreLibrary` | The picker's Photos grid (recent 120, only while the picker is open) and the Apps tab (behind decision 8's flag) |
| `SharedFiles`, `UriReader`, `ShareIntent` | Shared and picked URIs to picker items: `content:` only, names cleaned, duplicates dropped |
...
share parsing, direct-share target selection and ids, and the OEM screen table. Everything that talks to the system
