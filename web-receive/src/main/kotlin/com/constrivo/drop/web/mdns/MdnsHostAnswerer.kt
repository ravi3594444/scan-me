...
/**
 * The decision half of the mDNS responder (spec change N15): which queries for [hostName] get which answer. Pure,
 * so it is tested without sockets; [MdnsResponder] feeds it packets.
 *
...
class MdnsHostAnswerer(
    ...
    /** The reply to [query] received from [sourcePort], or null when it asks nothing we answer. */
    fun answer(
        query: DnsMessage,
        sourcePort: Int,
    ): Reply? {
        ...
        return if (legacy) {
            Reply(DnsMessage(query.id, flags, answered.map { it.copy(unicastResponse = false) }, answers, additionals = additionals), true)
        } else {
            Reply(DnsMessage(0, flags, answers = answers, additionals = additionals), answered.all { it.unicastResponse })
        }
    }

    /** The unsolicited announcement sent when the responder starts (RFC 6762 §8.3). */
