...
| WP7c+d Android Wi‑Fi links | Done | Wi‑Fi Direct owner and client (5 GHz when both support it, persistent groups for trusted pairs, the frequency check), legacy WPA2 join by network specifier, local-only hotspot with the system's real credentials, `SpecifierJoiner` with the 5 s restore budget (F‑E11), LAN link and NSD discovery, sockets bound to the link that dial only its addresses. 173 JVM tests on virtual time; nothing run on a device yet |
| WP7e+f Android service and storage | Done | `TransferService` (connectedDevice, plus dataSync while a transfer or the browser page runs; the Android 15 quota logged, S9), the `AndroidNode` composition root, MediaStore `IS_PENDING` storage (N14) with a SAF alternative, the SQLCipher database, thermal power policy and wake lock, progress, offer and completion notifications. JVM tests include two phones end to end over in-memory Bluetooth; the instrumented tests (SQLCipher, MediaStore, bound service) have not run, as there is no emulator here |
| Android app on the service | First device builds | `AppGraph` over the service (`TransferServiceClient`, `ServicePorts`), CameraX + ZXing scanning, the phone's Wi‑Fi rungs and NSD browse (`AndroidWifiStack`), the "Allow this computer?" prompt (N15), target API 36 until Android 17's local network permission is requested. Pairing fixes: a new install is visible to everyone for 10 min after onboarding and while "My code" is shown, the code expires with the beacon epoch it names, and the service is started as well as bound. The debug APK stores dex and native libraries compressed (28 MB), is signed with a repo debug key and is uploaded by CI. AirDrop-like transfer motion (design §4.2 note). Installed on a first phone on 24 Sep 2026; no two-phone result yet |
...
### Carried forward from WP7c–f and the Android app

- **CI:** two desktop tests time out on this branch (`DesktopNodeTest` "a flood of silent connections is capped…", `DesktopAppTest` "aFirstSendsCodeIsAskedForAgain…"); the cause is not diagnosed yet. The APK is still uploaded when they fail.
- **Android:** the phone does not announce itself over NSD, so a desktop cannot start a send to it; `WifiLinkListener.onLinkLost` and `onFrequencyChanged` are only logged, not routed to `LadderRunner`; the save-location picker (SAF tree) is not wired; on Android 12 the planner skips the Wi‑Fi rungs while their permission is missing (check the ask order in the lab); request `ACCESS_LOCAL_NETWORK` before the target moves to API 37.
- **Release:** R8 is still off (see WP7a+b), and a release build needs its own signing key, never the repo's debug key.
- **Lab:** the first two-phone runs: discovery and pairing, the Bluetooth head start and the Wi‑Fi Direct hop, notifications, Doze and OEM background limits.
