...
> - **mDNS (N15).** A minimal responder joins 224.0.0.251:5353 on the link interface only (source addresses outside its subnets are ignored), answers A (and ANY) queries for `drop.local` with the interface address, cache-flush bit and TTL 120 s, and answers AAAA with an NSEC negative response (RFC 6762 §6.1) so resolvers do not wait for an IPv6 address. Legacy unicast queries get the id and question back with TTL 10 s. It announces once on start and sends a goodbye on stop; it does not probe, since the phone's own network has no other `drop.local`.
> - **Page.** One HTML file (about 20 KB) with a CSP that allows exactly its inline script and style by SHA-256 hash. Downloads use `fetch` + `ReadableStream` with a progress bar and MB/s up to 1 GiB (the browser holds the bytes until the save) and fall back to plain links above that or when streams are unavailable.
