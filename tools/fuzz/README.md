...
frame), `bundle` (bundle index), `stream-open` (`StreamOpen` frames and the stream-id registry), `layout` (an `Offer`
and its `FileList` pages through `FileListAssembler` and `TransferLayout.of`, then `Resume`, `Retransmit` and
`Accept.resume` through `TransferLayout.expand`). The seed corpus is the golden vectors of
...
./gradlew :tools:fuzz:test -Pdrop.nightly=true  # nightly: 1 minute per target, seed from the date (never cached)
