...
class SecureSession internal constructor(
     ...
     *
     * Anyone on the network can connect to a listening port, so the `StreamOpen` must arrive within [timeoutMillis];
     * the caller closes [channel] on any failure.
     *
     * @throws ProtocolException for a missing, late, malformed, replayed or unauthenticated `StreamOpen`.
     */
    suspend fun acceptStream(
        channel: DataChannel,
        kind: LinkKind,
        timeoutMillis: Long = STREAM_OPEN_TIMEOUT_MILLIS,
    ): SecureConnection {
        val reader = FrameReader(channel, limits = FrameLimits.STREAM_START)
        var ended = false
        val frame =
            withTimeoutOrNull(timeoutMillis) { reader.readFrame().also { ended = it == null } }
                ?: throw ProtocolException(
                    if (ended) "data connection closed before its StreamOpen" else "no StreamOpen within $timeoutMillis ms",
                )
    ...
    companion object {
        internal val PRIMARY_STREAMS: IntArray = intArrayOf(ProtocolConstants.STREAM_ID_CONTROL, ProtocolConstants.STREAM_ID_BLUETOOTH)

        /** How long an accepted connection may take to send its `StreamOpen`. */
        const val STREAM_OPEN_TIMEOUT_MILLIS: Long = 5_000
    }
}
